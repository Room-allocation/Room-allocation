...
exports.addPermanentAssignment = async (req, res) => {
  try {
    const { dayOfWeek, startHour, endHour, room, lecturerName, subject } =
      req.body;
    ...
    const potentialTempConflicts =
      await TemporaryAssignment.find(timeOverlapQuery);
    ...
    res
      .status(500)
      .json({ message: "שגיאה בשרת בהוספת השיבוץ", error: error.message });
  }
};
...
exports.deletePermanentAssignment = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "שגיאה במחיקת השיבוץ", error: error.message });
  }
};
...
exports.addCancellation = async (req, res) => {
  console.log("Body received:", req.body); // הוסיפי את השורה הזו
  try {
    const { date, permanentAssignmentId, reason } = req.body;

    // בדיקה שהשיבוץ הקבוע קיים
    const assignment = await PermanentAssignment.findById(
      permanentAssignmentId,
    );
    if (!assignment) {
      return res
        .status(404)
        .json({ message: "Permanent assignment not found" });
    }
    ...
    if (existingCancellation) {
      return res
        .status(400)
        .json({ message: "Cancellation already exists for this date" });
    }
    ...
    // הוספה לחדר
    await Room.findByIdAndUpdate(assignment.room, {
...
exports.deleteCancellation = async (req, res) => {
  try {
    ...
    // הסרה מהחדר
    await Room.findByIdAndUpdate(cancellation.room, {
...
// שמירת שיבוץ זמני במונגו ועדכון החדר
// עדכון פונקציית העזר ב-assignmentController.js
async function saveTemporaryAssignment(data, message) {
  // חילוץ ידני של השדות מתוך האובייקט שהגיע מהבקשה
  const { date, startHour, endHour, room, subject, bookedBy } = data;

  const newTemp = new TemporaryAssignment({
    date,
    startHour,
    endHour,
    room,
    subject,
    // בנייה מחדש ומפורשת של האובייקט המקונן
    bookedBy: {
      operatorName: bookedBy.operatorName,
      phone: bookedBy.phone,
      eventType: bookedBy.eventType,
      classOrGroup: bookedBy.classOrGroup || ""
    }
  });
  ...
  // עדכון החדר המתאים עם ה-ID של השיבוץ החדש
...
exports.addTemporaryAssignment = async (req, res) => {
  console.log("--- בקשה חדשה הגיעה לשרת! ---"); 
  ...
  try {
    ...
    const permanentOnThisDay = await PermanentAssignment.find({
      room,
      dayOfWeek,
    });
    ...
    if (!permanentConflict) {
      const result = await saveTemporaryAssignment(
        req.body,
        "השיבוץ הזמני נוסף בהצלחה!",
      );
    ...
    const result = await saveTemporaryAssignment(
      req.body,
      "השיבוץ הזמני נוסף בהצלחה! (החדר היה פנוי בגלל ביטול שיבוץ קבוע)",
    );
    return res.status(201).json(result);
  } catch (error) {
